...
The data were obtained from the [PGA statistics website](https://www.pgatour.com/stats){target="_blank"}. Cases include all golfers who made the cut in each of 19 PGA tournaments in 2022. The data set variables for driving ability, putting ability, and measuring success in the tournament. The variables for measuring driving include average driving distance (`avgDriveDist`), driving accuracy percentage (`drivePct`), and strokes gained off the tee (`driveSG`). The variables for measuring putting are average putts per round (`avgPuttsPerRound`), one putt percentage (`onePuttPct`), and strokes gained putting (`puttsSG`). The measured success variables are scoring average (`avgScore`), official money won (`Money`), and Fedex Cup points (`Points`).
...
[PGA Correlations - no Tech](PGACorrelationsNoTech.docx) Addresses questions #1 and #2 and provides students with a correlation matrix.

[PGA Correlations - with Tech](PGACorrelationsWithTech.docx) Addresses questions #1 to #3 assuming students have technology to compute correlations and make scatterplots from the datafile.

## References

The data were obtained from the [PGA statistics website](https://www.pgatour.com/stats){target="_blank"}.
