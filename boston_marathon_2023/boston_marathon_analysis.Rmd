...
# 60-64
```{r}
men_60_64 <- lm(finish_net_sec ~ age_group, data = bostonMen)
newx = data.frame(age_group = "60-64")
predict(men_40_44, newdata = newx, interval = "conf", level = 0.90) # dif = 235.89

women_60_64 <- lm(finish_net_sec ~ age_group, data = bostonWomen)
newx = data.frame(age_group = "60-64")
predict(women_40_44, newdata = newx, interval = "conf", level = 0.90) # dif = 330.71

# 94.82 difference between men and women
```
# Conclusions so far 
...
This means that the runner times are separated by more seconds for women than for men at all ages which is somewhat interesting