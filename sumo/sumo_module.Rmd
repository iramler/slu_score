...
# Select
```{r}
# Rikishi is the ring name of the wrestler 
# Subset the data to contain only the variables Rikishi, weight_kg, and height_cm
sumo%>%
  select(Rikishi, weight_kg, height_cm)
```
...
```
```{r}
# Using one line, subset the data to contain only wrestlers from Mongolia or Tokyo 
sumo%>%
...
# Using one line, subset the data to contain only wrestlers from Mongolia who train at the heya called Kataonami
...
```{r}
# calculate total losses for each wrestler and order the resulting data from most to least
sumo%>%
  group_by(Rikishi)%>%
