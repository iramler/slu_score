...
plot(lm_AvgFinish)
```
Check the assumptions for the linear model. Do they seem reasonably met?
...
ggplot(data = nascar, aes(x = logDriverRating, y = AvgFinish))+
  ...
  geom_smooth(se = FALSE)
```
Linearity vastly improved. Looks like linearity assumption holds now.
...
```
Write down new equation and compare the adjusted R-squared values.
...
# Prediction (despite lack of indpendence) of Avg Finish for a log driver rating of 4.5
```{r}
