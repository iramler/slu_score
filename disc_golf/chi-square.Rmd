...
# Single proportion of pars for a course to determine the amount for the season
```{r}
disc_golf_holes <- read_csv("disc_golf_holes.csv")
disc<- disc_golf_holes%>%
  na.omit()
```
...
disc%>%
  ...
  summarise(pars = sum(`# Pars`),
            times_played = sum(`Times Played`))
```
# Sample Prop 
prop. of pars per times played = 3583/7254	= 0.49
...
#.49 +- 1.96 sqrt((.49(1-.49)))/7254

#.49 + .01 
#.49 - .01 
# We are 95% confident that the true proportion of pars on holes that MPO players compete on is between .48 and .50
```
